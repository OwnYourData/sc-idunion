Rails.application.routes.draw do
	...
	namespace :api, defaults: { format: :json } do
		scope module: :v1, constraints: ApiConstraints.new(version: 1, default: true) do
			...
			match 'meta/usage',         to: 'dpvs#show_usage',        via: 'get'
			...
			# IDunion specific
			match 'new_vaccination',        to: 'issuers#new',               via: 'post'
			match 'update_vaccination/:id', to: 'issuers#update',            via: 'put'
			match 'vc_status/:id',          to: 'issuers#check',             via: 'get'
			match 'vaccination_info',       to: 'issuers#vaccination_info',  via: 'get'
			match 'vaccination_apply',      to: 'issuers#vaccination_apply', via: 'post'
			match 'new_proof',              to: 'verifiers#new',             via: 'post'
			match 'update_proof/:id',       to: 'verifiers#update',          via: 'put'
			match 'proof_status/:id',       to: 'verifiers#check',           via: 'get'
			match 'scheduler/:schema',      to: 'processes#scheduler',       via: 'get'
		end
	end
	...
	match 'qr', to: 'connections#qr', via: 'get'
