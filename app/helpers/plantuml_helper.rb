module PlantumlHelper
    def plantuml(provision)
        begin
            ...
            @store = Store.find(provision["id"])
            if @store.schema_dri != VC_APPLY_DRI
                retVal += 'state "Activity: **issue Credential**" as a2 #palegreen' + "\n"
                retVal += "a2 : ts: " + @store.updated_at.utc.iso8601 + "\n"
                retVal += "a2 -> s1 : wasAssociatedWith\n"
                retVal += "a2 -> e1 : used\n"
